...
inline void normalize(M& out) {
  typename M::ValueType v;
  ACC::neutral(v);
  for (size_t n = 0; n < out.size(); ++n) ACC::op(out(n), v);
...
inline void weightedMean(const M& in1, const M& in2, const T alpha, M& out) {
  /// TODO
  /// Speedup
  T v1, v2;
...
inline void operate(const std::vector<BUFFER>& vec, M& out) {
  /// ???
  ...
  for (size_t j = 0; j < vec.size(); ++j) {
    ...
    for (size_t n = 0; n < out.size(); ++n) OP::op(b(n), out(n));
  }
}

/// out = op( out , op_i( hop(vec[i],rho[i]) ) )
template <class GM, class BUFFER, class M>
inline void operateW(const std::vector<BUFFER>& vec,
                     const std::vector<typename GM::ValueType>& rho, M& out) {
...
/// out = op( vec[0].current, ..., vec[i-1].current,vec[i+1].current, ... ,
/// vec[n].current )
template <class OP, class BUFVEC, class M, class INDEX>
inline void operate(const BUFVEC& vec, const INDEX i, M& out) {
  clean<OP>(out);
  /// TODO
  ...
  for (size_t j = 0; j < i; ++j) {
    ...
    for (size_t n = 0; n < out.size(); ++n) OP::op(f(n), out(n));
  }
...
inline void operateW(const BUFVEC& vec, const INDEX i,
                     const std::vector<typename GM::ValueType>& rho, M& out) {
  typedef typename GM::OperatorType OP;
...
struct OperateF_Functor {
  OperateF_Functor(const BUFVEC& vec, const INDEX i, ARRAY& out)
      : vec_(vec), i_(i), out_(out) {}
  ...
  void operator()(const FUNCTION& f) {
    ...
    if (f.dimension() == 2) {
      size_t count[2];
      typename GM::ValueType v;
      for (size_t n = 0; n < out_.size(); ++n) ACC::neutral(out_(n));
      if (i_ == 0) {
      ...
      for (size_t n = 0; n < f.shape(i_); ++n) ACC::neutral(out_(n));
      // factor shape iterator
      typedef typename FUNCTION::FunctionShapeIteratorType
          FunctionShapeIteratorType;
      opengm::ShapeWalker<FunctionShapeIteratorType> shapeWalker(
          f.functionShapeBegin(), f.dimension());
      for (IndexType scalarIndex = 0; scalarIndex < f.size();
           ++scalarIndex, ++shapeWalker) {
        ...
        for (IndexType j = 0; j < static_cast<typename GM::IndexType>(i_);
             ++j) {
          const LabelType label =
              static_cast<LabelType>(shapeWalker.coordinateTuple()[j]);
...
template <class GM, class ACC, class BUFVEC, class ARRAY, class INDEX>
inline void operateF(const typename GM::FactorType& f, const BUFVEC& vec,
                     const INDEX i, ARRAY& out) {
...
struct OperateWF_Functor {
  ...
  OperateWF_Functor(const ValueType rho, const BUFVEC& vec, const INDEX i,
                    M& out)
  ...
  void operator()(const FUNCTION& f) {
    ...
    // factor shape iterator
    typedef
        typename FUNCTION::FunctionShapeIteratorType FunctionShapeIteratorType;
    opengm::ShapeWalker<FunctionShapeIteratorType> shapeWalker(
        f.functionShapeBegin(), f.dimension());
    for (IndexType scalarIndex = 0; scalarIndex < f.size();
         ++scalarIndex, ++shapeWalker) {
...
template <class GM, class ACC, class BUFVEC, class M, class INDEX>
inline void operateWF(const typename GM::FactorType& f,
                      const typename GM::ValueType rho, const BUFVEC& vec,
                      const INDEX i, M& out) {
...
struct OperatorF2_Functor {
  ...
  typedef typename GM::ValueType ValueType;
  typedef typename GM::OperatorType OP;
  OperatorF2_Functor(const BUFVEC& vec, typename GM::IndependentFactorType& out)
      : vec_(vec), out_(out) {}
  ...
  void operator()(const FUNCTION& f) {
    ...
    for (IndexType scalarIndex = 0; scalarIndex < f.size();
      ...
      for (IndexType j = 0;
           j < static_cast<typename GM::IndexType>(vec_.size()); ++j) {
        const LabelType label =
            static_cast<LabelType>(shapeWalker.coordinateTuple()[j]);
        OP::op(vec_[j].current()(label), value);
      }
...
struct OperatorWF2_Functor {
  ...
  typedef typename GM::ValueType ValueType;
  typedef typename GM::OperatorType OP;
  OperatorWF2_Functor(ValueType rho, const BUFVEC& vec,
                      typename GM::IndependentFactorType& out)
      : rho_(rho), vec_(vec), out_(out) {}
  ...
  void operator()(const FUNCTION& f) {
    ...
    opengm::ShapeWalker<FunctionShapeIteratorType> shapeWalker(
        f.functionShapeBegin(), f.dimension());
    for (IndexType scalarIndex = 0; scalarIndex < f.size();
         ++scalarIndex, ++shapeWalker) {  // loop over the variables
      ValueType value;
      OP::ihop(f(shapeWalker.coordinateTuple().begin()), rho_, value);
      for (IndexType j = 0;
           j < static_cast<typename GM::IndexType>(vec_.size()); ++j) {
